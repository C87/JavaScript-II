...
let ticketPriceTotal = runners.reduce((tally, current) => {
    // debugger;
    return tally += current.donation;
}, 0);
...
// Problem 1 - Calcualte what percentage of ticketPriceTotal belongs to each runner

const donationShare = runners.map((runner) => {
    return {
        firstName: runner.first_name,
        lastName: runner.last_name,
        share: `${(runner.donation / ticketPriceTotal) * 100}%`,
    }
});

/* Remove % from runner.share string and check sum of all el.share is equal to 100
let total = 0;
donationShare.forEach((el)=> total += Number(el.share));
console.log(total); */
console.log(donationShare);

// Problem 2 - Create a mailing list

const mailingList = runners.map(runner => runner.email);

console.log(mailingList);

// Problem 3 - Group each runner by shirt size
...
Object.keys(sizes).forEach((size) => {
    sizes[size] = runners.filter(runner => runner.shirt_size === size );
})
